...
import 'package:brn/model/blacklist.dart';
import 'package:brn/model/member.dart';
import 'package:brn/presentation/component/shimmer_dark.dart';
import 'package:brn/presentation/screens/blacklist/blacklist_detail_screen.dart';
...
class _BlacklistScreenState extends State<BlacklistScreen> {
  ...
  List<BlacklistModel> member = [];
  getData() async {
    Uri url = Uri.parse(IpClass().getip2() + '/api/perpetrators');
    ...
    if (mounted) {
      setState(() {
        for (Map data in res['data']) {
          member.add(BlacklistModel.fromJson(data));
        }
        loading = false;
      });
    }
  ...
  Widget build(BuildContext context) {
    ...
    return CustomScaffold(
      ...
      appBar: [
        SimpleAppBarTitle(title: 'Blacklist'),
      ],
      body: Column(
